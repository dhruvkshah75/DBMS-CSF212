...
select distinct(productName) from products where productCode not in (select productCode from orderdetails) order by productName;

select customerName from customers where customerNumber in 
(select customerNumber from payments where amount >= all (select amount from payments));
...
where o.productCode is null order by p.productName;
...
INNER JOIN payments p ON c.customerNumber = p.customerNumber 
where p.checkNumber = 'CITI3434344' 
order by o.city;
